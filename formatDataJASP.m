...
JASPtable = table(Participants,EDA_before,TEMP_before,HR_before, ... 
    EDA_after,TEMP_after,HR_after, ...
    EDA_stop0,TEMP_stop0,HR_stop0, ...
    EDA_stop1,TEMP_stop1,HR_stop1, ...
    EDA_stop2,TEMP_stop2,HR_stop2, ...
    EDA_stop3,TEMP_stop3,HR_stop3, ...
    EDA_stop4,TEMP_stop4,HR_stop4, ...
    EDA_barefoot,TEMP_barefoot,HR_barefoot);
...
for k=1:length(subfolders)
    ...
    for i=1:length(sections)
        ...
        % column
        stdEDAslopes = std(EDA(:,2),'omitnan');

        % Compute the median of the HR and TEMP slopes and add to
        % column
        medianHRslopes = median(HR(:,2),'omitnan');
        medianTEMPslopes = median(TEMP(:,2),'omitnan');

        tableRow = [tableRow,stdEDAslopes,medianTEMPslopes,medianHRslopes];
    end
